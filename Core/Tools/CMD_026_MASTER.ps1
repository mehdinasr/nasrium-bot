...
Write-Host "NASRIUM - CMD_026_MASTER (System Scan & History)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
foreach($f in $files){
    $hash = ""
    # برای امنیت و سرعت، اگر فایل بزرگتر از 10 مگابایت نباشد هش محاسبه می‌شود
    if ($f.Length -lt 10MB) {
...
# ۵. نوشتن روی دیسک با کدگذاری UTF8
$masterOutput | ConvertTo-Json -Depth 100 | Out-File $outputFile -Encoding UTF8 -Force

Write-Host ""
Write-Host "========================================" -ForegroundColor Green
Write-Host "✅ CMD_026_MASTER SUCCESS" -ForegroundColor Green
Write-Host "📁 فایل نهایی با موفقیت ساخته شد:" -ForegroundColor White
Write-Host "   $outputFile" -ForegroundColor Yellow
Write-Host "📊 آمار ثبت شده:" -ForegroundColor White
Write-Host "   تعداد پوشه‌ها: $($folderList.Count)" -ForegroundColor Cyan
Write-Host "   تعداد فایل‌ها: $($fileList.Count)" -ForegroundColor Cyan
Write-Host "   گام‌های پیشرفت: $($completedCommands.Count)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Green
