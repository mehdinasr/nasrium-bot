# ===========================
# NOS_PRE_DISPATCHER_V2
# (generated by CMD_018F)
# ===========================
$__botLoaded = $false
try {
  . "D:\NASRIUM\Core\Modules\BotManager.ps1"
  $__botLoaded = $true
} catch {
  Write-Host ("BOTMANAGER LOAD ERROR: " + $_.Exception.Message) -ForegroundColor Red
}

if ($Command -like "bot-*") {
  if (-not $__botLoaded) {
    Write-Host "BOTMANAGER NOT LOADED. Cannot run bot commands." -ForegroundColor Red
    exit 2
  }
  switch ($Command) {
    "bot-status"  { Bot-Status; break }
    "bot-start"   { Bot-Start; break }
    "bot-stop"    { Bot-Stop; break }
    "bot-restart" { Bot-Restart; break }
    default { Write-Host "Unknown bot command" -ForegroundColor Yellow }
  }
  exit
}
param(
  [Parameter(Position=0)]
  [ValidateSet("status","backup","export","bot-status","bot-start","bot-stop","bot-restart","help")]
  [string]$Command = "help"
)
