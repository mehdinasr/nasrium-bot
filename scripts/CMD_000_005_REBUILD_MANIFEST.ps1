...
# File ID   : CMD_000_005
# Module    : Governance
# Component : Manifest Re-Seal after State Update
# Version   : 1.0.0
# Status    : Production
# ================================================================================
...
foreach ($name in $required) {
    ...
    $filesMeta += [ordered]@{ name = $name; version = "3.0.0"; sha256 = $hashStr; bytes = (Get-Item $p).Length; required = $true }
}
...
$manifest.files += [ordered]@{ name = "08_PROJECT_MANIFEST.json"; version = "3.0.0"; sha256 = $selfHash; bytes = $selfRaw.Length; required = $true }

$manifest | ConvertTo-Json -Depth 5 | Set-Content $ManifestPath -Encoding UTF8

Write-Host "✅ Manifest Re-Sealed Successfully." -ForegroundColor Green
