...
# File ID   : CMD_120_002
# Module    : Game
# Component : Town Hall Constraint Engine (Null Array Count Patch)
# Version   : 1.1.0
# Status    : Production
# ================================================================================
...
function Patch-THConstraint {
    Write-Host "=========================================" -ForegroundColor Cyan
    Write-Host "NASRIUM TH CONSTRAINT NULL ARRAY PATCH" -ForegroundColor Cyan
    Write-Host "Command   : CMD_120_002" -ForegroundColor Yellow
    Write-Host "=========================================" -ForegroundColor Cyan
    ...
    Write-Host "[CMD_120_002] Patching NSM_GameEngine.psm1..." -ForegroundColor Cyan
    ...
    try {
        ...
        Write-Host "  [OK] Game Engine Patched." -ForegroundColor Green
    } catch {
        throw "Failed to patch Engine: $_"
    }

    Write-Host "[CMD_120_002] Running Integration Tests..." -ForegroundColor Cyan
    try {
    ...
    Write-Host "[CMD_120_002] Updating Registry..." -ForegroundColor Cyan
    try {
        $RegistryPath = "$Root\Core\Registry"
        $stampISO = Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ"
        $state = @{
            cmd_id = "CMD_120_002"
            task = "Town Hall Constraint Engine (Null Array Count Patch)"
            status = "COMPLETED"
            completed_at = $stampISO
            next_step = "CMD_121"
        } | ConvertTo-Json -Depth 3
        [System.IO.File]::WriteAllText((Join-Path $RegistryPath "CMD_120_002_state.json"), $state, [System.Text.Encoding]::UTF8)
...
if (Get-Command Invoke-NasriumCommand -ErrorAction SilentlyContinue) {
    Invoke-NasriumCommand -CmdId "CMD_120_002" -Action { Patch-THConstraint }
} else {
    Patch-THConstraint
}
