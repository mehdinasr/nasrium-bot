...
# Component : Auto Tunnel Refresh & Telegram Button
# Version   : 1.0.0
# Status    : Production
# ================================================================================
...
$logFile = Join-Path $logsPath "CMD_148_$timestamp.log"
...
Write-Log "========================================" "HEADER"
Write-Log "NASRIUM CMD_148 - AUTO TUNNEL REFRESH" "HEADER"
Write-Log "========================================" "HEADER"

# --- CHECK SERVER ---
Write-Log "--- Checking Server ---" "STEP"
try {
    $r = Invoke-WebRequest -Uri "http://localhost:8080/api/player/TEST_USER_001" -Method Get -TimeoutSec 3 -ErrorAction Stop
...
# --- GET TUNNEL URL ---
Write-Log ""
Write-Log "--- Tunnel URL Input ---" "STEP"
Write-Log "Make sure START_TUNNEL.bat is running" "INFO"
Write-Log "Copy the https:// URL from tunnel window" "INFO"
Write-Log ""

$TunnelUrl = Read-Host "Paste tunnel URL here"

if ([string]::IsNullOrWhiteSpace($TunnelUrl)) {
    Write-Log "No URL provided. Exiting." "ERROR"
    exit 1
}

$TunnelUrl = $TunnelUrl.Trim().TrimEnd('/')
Write-Log "Using: $TunnelUrl" "SUCCESS"
...
$JsContent = $JsContent -replace 'const API_BASE = "https://[^"]+/api";', "const API_BASE = `"$TunnelUrl/api`";"
$JsContent = $JsContent -replace 'const API_BASE = "http://localhost:8080/api";', "const API_BASE = `"$TunnelUrl/api`";"
...
<head>
<meta charset="UTF-8">
<title>NASRIUM - Send Button</title>
<style>
...
const body={chat_id:chatId,text:'🎮 *NASRIUM is ONLINE!*\\n\\nClick below to enter:',parse_mode:'Markdown',reply_markup:{inline_keyboard:[[{text:'🚀 OPEN NASRIUM',web_app:{url:'$WebUrl'}}]]}};
try{const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await res.json();if(data.ok){showStatus('✅ SUCCESS! Check Telegram.','success')}else{showStatus('❌ Error: '+data.description,'error')}}catch(e){showStatus('❌ Network error. Turn ON VPN.','error')}}
...
$OutHtml = Join-Path $projectRoot "SEND_BUTTON_QUICK.html"
...
Write-Log ""
Write-Log "========================================" "HEADER"
Write-Log "CMD_148 COMPLETE" "HEADER"
Write-Log "========================================" "HEADER"
Write-Log "✅ Server: RUNNING" "SUCCESS"
Write-Log "✅ Tunnel: $TunnelUrl" "SUCCESS"
Write-Log "✅ Configs: UPDATED" "SUCCESS"
Write-Log "✅ Chrome: OPENED" "SUCCESS"
Write-Log ""
Write-Log "NEXT:" "HEADER"
Write-Log "1. Turn ON VPN in Chrome" "INFO"
Write-Log "2. Click 📤 SEND GAME BUTTON" "INFO"
Write-Log "3. Go to Telegram → click 🚀 OPEN NASRIUM" "INFO"
Write-Log ""
Write-Log "⚠️ KEEP OPEN:" "WARNING"
Write-Log "   - START_NASRIUM_SERVER.ps1" "WARNING"
Write-Log "   - START_TUNNEL.bat" "WARNING"
