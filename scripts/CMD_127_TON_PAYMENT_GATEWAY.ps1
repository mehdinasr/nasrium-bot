...
# File ID   : CMD_127_001
# Module    : Web3
# Component : TON Real-Payment Gateway Engine
# Version   : 1.0.0
# Status    : Production
# ================================================================================
...
function Build-TonPaymentGateway {
    ...
    Write-Host "Command   : CMD_127" -ForegroundColor Yellow
    Write-Host "=========================================" -ForegroundColor Cyan
...
"@
    ...
    try {
        ...
        $state = @{
            cmd_id = "CMD_127_001"
            task = "TON Real-Payment Gateway Engine"
            status = "COMPLETED"
            completed_at = $stampISO
            next_step = "CMD_128"
        } | ConvertTo-Json -Depth 3
        [System.IO.File]::WriteAllText((Join-Path $RegistryPath "CMD_127_001_state.json"), $state, [System.Text.Encoding]::UTF8
...
if (Get-Command Invoke-NasriumCommand -ErrorAction SilentlyContinue) {
    Invoke-NasriumCommand -CmdId "CMD_127" -Action { Build-TonPaymentGateway }
} else {
